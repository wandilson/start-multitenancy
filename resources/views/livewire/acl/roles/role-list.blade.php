<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Lista de Funções Administrativas') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <a href="{{ route('roles.create') }}" class="border bg-blue-400 text-white rounded-md px-4 py-2 transition duration-500 ease select-none hover:text-white hover:bg-blue-400 focus:outline-none focus:shadow-outline">
                Nova Função
            </a>
        </div>
        <br>
                                    ...
                                    @foreach ($roles as $role)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <a href="{{ route('roles.edit', $role->id) }}" class="border border-yellow-300 bg-yellow-300 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-yellow-400 focus:outline-none focus:shadow-outline">
                                                    Editar
                                                </a>
                                                <a href="javascript:void(0)" wire:click.prevent="destroy({{ $role->id }})" onclick="confirm('Deseja realmente remover esse registro? Todos os dados relacionados serão removidos!') || event.stopImmediatePropagation()" class="border border-red-400 bg-red-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-red-600 focus:outline-none focus:shadow-outline">
                                                    Deletar
                                                </a>
                                            </td>
                                        </tr> 
