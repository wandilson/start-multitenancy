<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Permissão para o Módulo:') }} <span class="text-red-500 uppercase">{{ $permission->module->name}}</span>
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <a href="{{ route('module.permissions', $permission->module_id) }}" class="border bg-blue-400 text-white rounded-md px-4 py-2 transition duration-500 ease select-none hover:text-white hover:bg-blue-400 focus:outline-none focus:shadow-outline">
                Voltar
            </a>
        </div>
        <br>
